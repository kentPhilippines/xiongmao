...
public class PagePlugin implements Interceptor{
	...
	private static String pageSqlId = ".*select.*"; 
	
	public static Dialect getDialectObject() {
		return dialectObject;
	}

	public static void setDialectObject(Dialect dialectObject) {
		PagePlugin.dialectObject = dialectObject;
	}
	...
	public Object intercept(Invocation ivk) throws Throwable {
		if (ivk.getTarget() instanceof RoutingStatementHandler) {
			...
			MappedStatement mappedStatement = (MappedStatement) ReflectHelper.getValueByFieldName(delegate, "mappedStatement");//获取接口名
			/**
			 * 方法1：通过ＩＤ来区分是否需要分页．.*select.*
			...
			if (mappedStatement.getId().matches(pageSqlId)) { // 拦截需要分页的SQL
				...
				if (parameterObject == null) {
					...
					java.sql.PreparedStatement countStmt = null;//预编译对象
					ResultSet rs = null;
					try {
